...
async function fetchLiveInventory(): Promise<TransformedProduct[]> {
  try {
    // Call our safe internal Next.js API route
    // Using an absolute URL pattern ensures it works perfectly on Vercel deployment environments
    ...
    console.error("Could not reach internal inventory API:", err);
    return [];
  }
}
    ...
    <div className="space-y-12 py-8 max-w-5xl mx-auto">
      ...
      <section className="text-center space-y-4">
        ...
        <p className="text-gray-400 font-mono text-sm max-w-2xl mx-auto">
          Authorized community personnel may requisition custom physical gear. All assets are manufactured on-demand and shipped directly to your terminal coordinates.
        </p>
      </section>
      ...
      {products.length === 0 ? (
        <div className="panel-slf p-8 text-center rounded-xl border border-dashed border-gray-800 text-gray-500 font-mono text-sm">
          ⚠️ Terminal Alert: No active inventory streams detected. Ensure products are pushed to your Printful store container.
        </div>
      ) : (
        <section className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
          {products.map((product) => (
            <div key={product.id} className="panel-slf flex flex-col p-5 rounded-xl space-y-4 anime-glitch-hover bg-black/60 border border-gray-800 group transition-all hover:border-jade-tech/50">
              ...
              {/* Title & Out-of-the-Box Requisition Trigger */}
                ...
                <div className="pt-3 border-t border-gray-800 flex justify-end">
                  {product.externalUrl ? (
                    <a 
                      ...
                      className="w-full text-center px-4 py-2 bg-jade-tech/10 hover:bg-jade-tech/20 text-jade-tech border border-jade-tech/50 rounded font-mono text-xs uppercase tracking-widest transition-all"
                    >
                      Acquire Asset ↗
                    </a>
                  ) : (
                    <button 
                      disabled
                      className="w-full text-center px-4 py-2 bg-gray-900 text-gray-600 border border-gray-800 rounded font-mono text-xs uppercase tracking-widest cursor-not-allowed"
                    >
                      Awaiting Link
                    </button>
                  )}
